...
namespace Bricklayer.Core.Client.Interface.Screens
{
    internal class GameScreen : Screen
    {
        ...
        public override void Add(ScreenManager screenManager)
        {
            ...
            Window.Add(lstChats);
            for (var i = 0; i < lstChats.Height / 16; i++) //Hack to make chat start at bottom (easier than modifying rendering code)
                lstChats.Items.Add(new ChatDataControl(string.Empty, Manager, lstChats));
        }
        ...
        private void HandleInput()
        {
            ...
            else if ((Client.Input.IsKeyPressed(Keys.Enter) && txtChat.Visible) || Client.Input.IsKeyPressed(Keys.Escape))
            {
                ...
                {
                    AddChat(txtChat.Text, Manager, lstChats);
